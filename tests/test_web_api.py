...
class TestAnalyzeEndpoint:
    ...
    def test_analyze_with_valid_session(self):
        ...
        response = self.client.post(f"/api/v1/analyze?session_id={self.session_id}")
    ...
    def test_analyze_with_epsilon(self):
        ...
        response = self.client.post(f"/api/v1/analyze?session_id={self.session_id}&epsilon=0.5")
    ...
    def test_analyze_epsilon_boundaries(self):
        ...
        # Too low
        response = self.client.post(f"/api/v1/analyze?session_id={self.session_id}&epsilon=0.001")
        assert response.status_code == 422  # Validation error

        # Too high
        response = self.client.post(f"/api/v1/analyze?session_id={self.session_id}&epsilon=15")
    ...
    def test_analyze_invalid_strategy(self):
        ...
        response = self.client.post(f"/api/v1/analyze?session_id={self.session_id}&anonymization_strategy=invalid")
        assert response.status_code == 422

    def test_analyze_with_quasi_identifiers(self):
        """Test analysis with quasi-identifiers."""
        response = self.client.post(f"/api/v1/analyze?session_id={self.session_id}&quasi_identifiers=age,city")
...
class TestGenerateReportEndpoint:
    ...
    def test_generate_report_returns_html(self):
        ...
        response = self.client.post(f"/api/v1/generate-report?session_id={self.session_id}")
...
class TestAPIIntegration:
    ...
    def test_full_workflow(self):
        ...
        # Step 2: Analyze
        analyze_response = self.client.post(f"/api/v1/analyze?session_id={session_id}&epsilon=1.0")
        assert analyze_response.status_code == 200

        # Step 3: Generate Report
        report_response = self.client.post(f"/api/v1/generate-report?session_id={session_id}")
    ...
    def test_multiple_uploads(self):
        ...
        response1 = self.client.post(
            "/api/v1/upload", files={"file": ("file1.csv", io.BytesIO(csv_content1), "text/csv")}
        )
        response2 = self.client.post(
            "/api/v1/upload", files={"file": ("file2.csv", io.BytesIO(csv_content2), "text/csv")}
        )
...
class TestErrorHandling:
    ...
    def test_empty_file_upload(self):
        ...
        # Empty file will cause pandas to raise an exception
        # The test just verifies the endpoint doesn't crash unexpectedly
        try:
            response = self.client.post("/api/v1/upload", files=files)
            # If it returns, should be an error status
            assert response.status_code in [400, 422, 500]
        except Exception:
            # Expected - pandas throws EmptyDataError
            pass
