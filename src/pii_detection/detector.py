...
class PIIDetector:
    ...
    def _get_enabled_entities(self) -> List[str]:
        ...
        return [entity for entity, config in self.config.get("entities", {}).items() if config.get("enabled", True)]
    ...
    def get_pii_summary(self, text: str) -> Dict[str, Any]:
        ...
        summary = {"total_pii": len(results), "by_type": {}}

        for result in results:
            entity_type = result["entity_type"]
            if entity_type not in summary["by_type"]:
                summary["by_type"][entity_type] = {"count": 0, "instances": []}
