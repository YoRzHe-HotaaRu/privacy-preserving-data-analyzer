...
from src.data_ingestion import CSVLoader, load_file
...
from src.risk_assessment import ComplianceChecker, RiskCalculator

# Initialize FastAPI
app = FastAPI(
    title="Privacy-Preserving Data Analyzer", description="Analyze data while protecting privacy", version="1.0.0"
)
...
async def upload_file(file: UploadFile = File(...)):
    ...
    try:
        ...
        for col in df.columns:
            ...
            if col_pii:
                pii_results[col] = {"count": len(col_pii), "types": list(set(p["entity_type"] for p in col_pii))}

        # Store in session
        session_id = datetime.now().strftime("%Y%m%d%H%M%S")
        session_data[session_id] = {"df": df, "filename": file.filename, "pii_results": pii_results}
