...
class DataAnonymizer:
    ...
    def anonymize_dataframe(self, df: pd.DataFrame, 
                          pii_columns: Dict[str, str]) -> pd.DataFrame:
        """
        ...
        for column, entity_type in pii_columns.items():
            if column in anonymized_df.columns:
                ...
                anonymized_df[column] = anonymized_df[column].apply(
                    lambda x: self.apply_strategy(str(x), entity_type, strategy)
                    if pd.notna(x) else x
                )
        
        return anonymized_df
    
    def anonymize_dataframe_auto(self, df: pd.DataFrame, 
                                pii_detector=None) -> tuple:
        """
        ...
        for column in df.columns:
            ...
            if all_pii:
                ...
                anonymized_df[column] = anonymized_df[column].apply(
                    lambda x: self.apply_strategy(str(x), most_common_entity, strategy)
                    if pd.notna(x) else x
                )
